...
#define CHASSIS_VX_RC_SEN 0.1f //changed 2023.2.5 use the raw data of RC
//rocker value (max 660) change to horizontal speed (m/s)
//遥控器左右摇杆（max 660）转化成车体左右速度（m/s）的比例
//#define CHASSIS_VY_RC_SEN 0.005f
#define CHASSIS_VY_RC_SEN 0.1f //changed 2023.2.5 use the raw data of RC
...
#define CHASSIS_ANGLE_YAW_RC_SEN 0.136f
...
//单个底盘电机最大速度
#define MAX_WHEEL_SPEED 29000.0f // changed 2023.2.11 4.0f
...
//底盘电机速度环PID
//#define M3505_MOTOR_SPEED_PID_KP 15000.0f // comment 2023.2.6
//#define M3505_MOTOR_SPEED_PID_KI 10.0f //comment 2023.2.6
#define M3505_MOTOR_SPEED_PID_KP 1.0000f // changed 2023.2.6 for 3510 chasis
#define M3505_MOTOR_SPEED_PID_KI 0.00001f	// changed 2023.2.6 for 3510 chasis
#define M3505_MOTOR_SPEED_PID_KD 0.0f
#define M3505_MOTOR_SPEED_PID_MAX_OUT MAX_MOTOR_CAN_CURRENT
#define M3505_MOTOR_SPEED_PID_MAX_IOUT 2000.0f
//地盘电机XiaoMi速度环PID // added 2023.6.21
#define Mi_MOTOR_SPEED_PID_KP 1.0000f // changed 2023.2.6 for 3510 chasis
#define Mi_MOTOR_SPEED_PID_KI 0.00000f	// changed 2023.2.6 for 3510 chasis
#define Mi_MOTOR_SPEED_PID_KD 0.0f
#define Mi_MOTOR_SPEED_PID_MAX_OUT MAX_MOTOR_CAN_CURRENT
#define Mi_MOTOR_SPEED_PID_MAX_IOUT 2000.0f
...
typedef struct
{
  const motorMi_measure_t *chassis_motorMi_measure;
  fp32 accel;
  fp32 speed;
  fp32 speed_set;
	fp32 pos;
...
typedef struct
{
  ...
  const fp32 *chassis_INS_angle;             //the point to the euler angle of gyro sensor.获取陀螺仪解算出的欧拉角指针
	const fp32 *chassis_INS_gyro;							 //the point to the gyro of gyro sensor.获取陀螺仪反馈的角速度指针
	const fp32 *chassis_INS_accel;						 //the point to the acceleration of gyro sensor.获取陀螺仪反馈的加速度指针
	const fp32 *Yaw_Axis_angle;								//the point to the yaw axis angle of encoder.获取编码器测量出的Yaw轴角度指针
  chassis_mode_e chassis_mode;               //state machine. 底盘控制状态机
  chassis_mode_e last_chassis_mode;          //last state machine.底盘上次控制状态机
  chassis_motorMi_t motorMi_chassis[2];          //chassis motor data.底盘电机数据
  ...
  first_order_filter_type_t chassis_cmd_slow_set_yaw;  //use first order filter to slow set-point.使用一阶低通滤波减缓设定值
  ...
  fp32 chassis_roll;  //the roll angle calculated by gyro sensor and gimbal motor.陀螺仪和云台电机叠加的roll角度
	fp32 chassis_gyro[3]; //the gyro by gyro sensor. 从陀螺仪获得的角速度；
	fp32 chassis_accel[3]; //the acceleration by gyro sensor. 从陀螺仪获得的加速度；
	fp32 yaw_axis_angle; //the angle of yaw axis measured by encoder. Yaw轴与底盘的角度
...
/**
  ...
  * @retval         none
  */
extern void chassis_rc_to_control_vector(fp32 *vx_set, fp32 *vy_set, fp32 *yaw_set, chassis_move_t *chassis_move_rc_to_vector);
